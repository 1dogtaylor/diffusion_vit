...
from .layers import PatchEmbedding, AddCLSToken, AddPositionEmbedding, TransformerEncoder, AddCLSToken_diffusion
from .config import BASIC_URL, WEIGHTS, VIT_B_CONFIG, VIT_B_CONFIG_DIFFUSION, VIT_L_CONFIG, WEIGHTS_CONFIG, PRE_TRAINED_WEIGHTS_FNAME
...
class Diffusion_ViT_cnn(models.Model):
	"""Implementation of VisionTransformer Based on Keras
	Args:
		- image_size: Input Image Size, integer or tuple
		- patch_size: Size of each patch, integer or tuple
	...
	def build(self, input_shape):
		...
		self.layer_norm = layers.LayerNormalization(epsilon=1e-6, name="layer_norm")
		if self.pre_logits: self.pre_logits = layers.Dense(self.hidden_dim, activation="tanh", name="pre_logits")
		if self.cnn_decoder: self.cnn_decoder = self.create_conv_decoder(input_shape[0])
		super().build(input_shape)
