...
def ddpm(x_t, pred_noise, t):
    alpha_t = alpha[t]  # Direct indexing
    alpha_t_bar = alpha_bar[t]  # Direct indexing

    eps = 1e-8  # Small epsilon to prevent division by zero
    eps_coef = (1 - alpha_t) / ((1 - alpha_t_bar) + eps) ** 0.5
...
if train:
    ...
    for step in range(255):
        ...
        ts = np.array([[step / 255.0]])
        pred_noise = predict(x, ts)
        x = ddpm(x, pred_noise, step) # bc step needs to be int not normallized float
        images.append(x)
        print(f'\rstep: {step+1}/256', end='', flush=True)
    
    images = [tf.cast(img*255, dtype=tf.uint8) for img in images]
